...
const HorizontalCourseNode = memo(({ data }: HorizontalCourseNodeProps) => {
	...
	return (
		<div
			className={`border-2 border-black bg-white p-2 rounded w-80 h-28 flex flex-col justify-between hover:cursor-move relative`}
		>
			<Handle
				type="target"
			...
			<div className="flex justify-center items-center">
				<div
					className={`relative bg-${data.color} h-4 w-full text-center text-xs rounded mx-1`}
				>
					<span className="absolute top-0 left-0 right-0">
						{data.label && data.color !== 'black' && `${data.label}`}
					</span>
				</div>
			</div>
			<Handle
